...
try:
    for search_term in search_terms:
        for i, tweet in enumerate(sntwitter.TwitterSearchScraper(f'{search_term} since:{search_year}-01-01 until:{search_year}-12-31').get_items()):
            ...
            tweets_list.append([tweet.date.year, tweet.id, content, tweet.url, tweet.user.username, location, followers, following, verified, tweets_source, tweet.likeCount, tweet.retweetCount, tweet.replyCount])
...
with open(f"{filename}.csv", mode='w', newline='', encoding='utf-8') as file:
    ...
    writer.writerow(['Year', 'ID', 'Content', 'URL', 'Username', 'Location', 'Followers', 'Following', 'Verified', 'Source', 'Likes', 'Retweets', 'Replies'])  # header row
    writer.writerows(tweets_list)

# print the first 10 tweets
print(f"\nHere are the first {min(10, num_tweets)} tweets:")
for tweet in tweets_list[:min(10, num_tweets)]:
